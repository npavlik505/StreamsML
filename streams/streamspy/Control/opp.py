...
class controller(BaseController):
    ...
    def recompute_obs(self) -> None:
        ...
        if self.org_motion == "undefined":
            ...
            sensor_ydelta_loc = 10 / self.TargetReFriction
            sensor_y_loc = int(np.abs(y_delta - sensor_ydelta_loc).argmin())

            # .5 delta in grid points
            sensor_len_bl = 0.5
            sensor_x_len = math.ceil((sensor_len_bl / self.lx) * self.nx)

            # slot length > sensor > zero
            sensor_x_len = max(3, sensor_x_len)
            sensor_x_len = min(sensor_x_len, self.slot_end - self.slot_start)
    ...
    def compute_action(self, observation):
        ...
        # Purged ema for better signal
        v_avg = float(np.mean(observation))
        output = -self.gain * v_avg
